...
export default function Blogs () {
  ...
  const handleChangeEmphasis = (row: IBlogs) => {
    toast.info('Aguarde um instante', {
      position: "top-right",
      pauseOnHover: false,
      autoClose: false,
    ...
    BASE_URL_V2.get(`/blog-emphasis/${row.id}`)
      .then(() => {
        toast.dismiss()
        toast.success('Blog alterado com sucesso!', {
          position: "top-right",
          pauseOnHover: false,
          autoClose: false,
        });
      ...
      .catch(() => {
        toast.dismiss()
        toast.error('Erro ao alterar o blog', {
          position: "top-right",
          pauseOnHover: false,
          autoClose: 5000
        });
  ...
  const UsersColumns: GridColDef[] = [
    {
      ...
      renderCell: ({ row } : {row: IBlogs}) =>{
        return TableActions({
        editRoute: `/blogs/post/editar-blog/${row.id}`,
    ...
    },
    {
      ...
      renderCell: ({ row } : {row: IBlogs}) => (
        <Switch
          ...
          onChange={() => handleChangeEmphasis(row)}
        />
      ),
      sortable: false,
