...
export default function HomeClients () {
  ...
  const { control, handleSubmit } = useForm<IHomeClients | FieldValues>({
    defaultValues: async () => {
      return await BASE_URL_V2.get<IHomeClients>('/get-clients-text')
      .then(({data}) => ({
        ...data, 
      }))
  ...
  const onSubmit = (data: IHomeClients | FieldValues) => {
    setLoading(true)
    if(editForm) {
      BASE_URL_V2.put('/edit-clients-text', {
        ...data,
      })
        .then(() => {
          toast.dismiss()
        ...
        BASE_URL_V2.post('/register-clients-text', {
          ...data,
        })
          .then(() => {
            toast.dismiss()
