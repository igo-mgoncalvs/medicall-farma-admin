...
export default function AboutUsTeam () {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitted } } = useForm<IAboutUsTeam | FieldValues>({
    defaultValues: async () => {
      return await BASE_URL_V2.get<IAboutUsTeam>('/get-team')
      .then(({data}) => ({
        ...data, 
      }))
  ...
  const onSubmit = useCallback((data: IAboutUsTeam | FieldValues) => {
    setLoading(true)

    if(editForm) {
      BASE_URL_V2.put('/edit-team', {
        ...data,
      })
        .then(() => {
          toast.dismiss()
        ...
        BASE_URL_V2.post('/register-team', {
          ...data,
        })
          .then(() => {
            toast.dismiss()
