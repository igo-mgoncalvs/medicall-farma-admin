...
export default function AboutUsBanners () {
  ...
  const getData = () => {
    BASE_URL.get<IAboutUs_Banners[]>('about-us-banners')
      .then(({data}) => {
        setList(data)
      })
  }
  ...
  const handleDeleteImage = (image: IAboutUs_Banners) => {
    ...
    BASE_URL.post(`/about-us-delete-banner/${image.imageId}`, {
  ...
  const handleUpdateImageList = useCallback(() => {
    ...
    BASE_URL.put('/about-us-update-banners', list)
      .then(() => {
        toast.dismiss()
