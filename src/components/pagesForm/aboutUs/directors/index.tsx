...
export default function AboutUsDirectors () {
  ...
  const getData = () => {
    BASE_URL.get<IAboutUs_Values[] | []>('/about-us-directors')
      .then(({ data }) => {
        setRows(data)
      })
  }
  ...
  const onSubmit = useCallback((data: IAboutUs_Values) => {
    ...
    if(!valueEdit) {
      ...
      BASE_URL.post('/about-us-add-directors', data)
        .then(() => {
          toast.dismiss()
      ...
      BASE_URL.put(`about-us-edit-directors/${data.id}`, data)
        .then(() => {
          toast.dismiss()
